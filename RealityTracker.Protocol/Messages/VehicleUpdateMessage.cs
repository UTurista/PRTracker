...
public readonly record struct VehicleUpdateMessage : IMessage
{
    ...
    internal static VehicleUpdateMessage Create(short messageLength, CounterBinaryReader reader)
    {
        ...
        {
            var vehicleUpdateFlags = (Flags)reader.ReadByte();
            var vehicleID = reader.ReadInt16();
            byte? team = vehicleUpdateFlags.HasFlag(Flags.Team) ? reader.ReadByte() : null;
            Vector3? position = vehicleUpdateFlags.HasFlag(Flags.Position) ? reader.ReadVector3() : null;
            short? yawRotation = vehicleUpdateFlags.HasFlag(Flags.Rotation) ? reader.ReadInt16() : null;
            short? health = vehicleUpdateFlags.HasFlag(Flags.Health) ? reader.ReadInt16() : null;
